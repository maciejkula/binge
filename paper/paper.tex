...
Industry ranking and recommendation systems need to scale to tens or hundreds of millions of items and users. As the number of items available to be recommended grows large, the challenge of etimating models from large amounts of interaction data is joined by the challenge of efficiently ranking the item catalogue to produce top-ranked recommendations. With large catalogues, ranking latency (in on-line settings) and compute and storage costs (in pre-compute settings) place significant contraints on the design of the entire recommender system.
